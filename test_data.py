...
for i in range(x_train.shape[1]):
    _min = np.min(x_train[:, i])  # 每一列的最小值
    _max = np.max(x_train[:, i])  # 每一列的最大值
    x_train[:, i] = (x_train[:, i] - _min) / (_max - _min)  # 归一化到0-1之间

for i in range(y_train.shape[1]):
    _min = np.min(y_train[:, i])  # 每一列的最小值
    _max = np.max(y_train[:, i])  # 每一列的最大值
    y_train[:, i] = (y_train[:, i] - _min) / (_max - _min)  # 归一化到0-1之间

for i in range(x_test.shape[1]):
    _min = np.min(x_test[:, i])  # 每一列的最小值
    _max = np.max(x_test[:, i])  # 每一列的最大值
    x_test[:, i] = (x_test[:, i] - _min) / (_max - _min)  # 归一化到0-1之间
    
for i in range(y_test.shape[1]):
    _min = np.min(y_test[:, i])  # 每一列的最小值
    _max = np.max(y_test[:, i])  # 每一列的最大值
    y_test[:, i] = (y_test[:, i] - _min) / (_max - _min)  # 归一化到0-1之间
'''

a = HiddenLayer(x_train_1, 90)  # data_X
...
for i in result:
    ...
    #cost = np.power(infer_result - ground_truth, 2)/ground_truth
